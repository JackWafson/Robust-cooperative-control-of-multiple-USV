...
class PolicyNet(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim):
        ...
        torch.nn.init.uniform_(self.fc_mu.weight, a=-0.1, b=0.1)
        torch.nn.init.uniform_(self.fc_std.weight, a=-0.1, b=0.1)
    def forward(self, x):
        s_1 = x
        x = F.relu(self.fc1(x))
        s_2 = x
...
class QValueNet(torch.nn.Module):
    def __init__(self, state_dim, hidden_dim, action_dim):
        ...
        self.fc2 = torch.nn.Linear(state_dim + action_dim, hidden_dim)
        self.fc3 = torch.nn.Linear(hidden_dim, 1)

    def forward(self, x, a):
        cat = torch.cat([x, a], dim=1)  # 拼接状态和动作
        x = F.relu(self.fc1(cat))
        x = F.relu(self.fc2(cat))
        return self.fc3(x)
...
class SAC:
    ...
    def __init__(self, state_dim, hidden_dim, action_dim,
        ...
        self.tau = tau
        self.device = device
        self.buffer = ReplayBuffer(buffer_size=int(1e6), batch_size=128, device=torch.device("cuda"))
    ...
    def save(self):
        torch.save(self.actor.state_dict(), './model.pth')
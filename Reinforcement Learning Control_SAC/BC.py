...
def BC(w_last, env_r, env, t):
    '''反步控制策略'''

    s_r = env_r.state    
    s = env.state

    z_1_1 = s[0] - s_r[0] #- np.clip(0.2*math.sin(0.1*t),-0.2, 0.2)
    z_1_2 = s[1] - s_r[1] #- np.clip(0.2*math.cos(0.1*t),-0.2, 0.2)
    z_1_3 = s[2] - s_r[2]
    if z_1_3 > np.pi:
        z_1_3 = z_1_3 - 2 * np.pi
    elif z_1_3 < -np.pi:
    ...
    tau = cdv + np.dot(M, w_dot) - np.dot(K_2, z_2) - 50*np.sign(z_2)
